        ...
        return (
          <li key={href}>
            <Link
              ...
              passHref
              className={`${
                isActive ? "bg-green-300 shadow-md" : ""
              } hover:bg-green-300 hover:shadow-md focus:!bg-green-300 active:!text-neutral py-1.5 px-3 text-sm rounded-full gap-2 grid grid-flow-col`}
            >
              {icon}
              <span>{label}</span>
            </Link>
...
export const Header = () => {
  ...
  return (
    <div className="sticky lg:static top-0 navbar bg-green-400 min-h-0 flex-shrink-0 justify-between z-20  px-0 sm:px-2">
      <div className="navbar-start w-auto lg:w-1/2">
        <div className="lg:hidden dropdown" ref={burgerMenuRef}>
          <label
            tabIndex={0}
            className={`ml-1 btn btn-ghost ${isDrawerOpen ? "hover:bg-green-300" : "hover:bg-transparent"}`}
            onClick={() => {
        ...
        <Link href="/" passHref className="hidden lg:flex items-center gap-2 ml-4 mr-6 shrink-0">
          ...
          <div className="flex flex-col">
            <span className="font-lighter leading-tight">Bloom</span>
          </div>
        </Link>
